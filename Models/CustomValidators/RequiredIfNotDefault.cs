...
{
    public class RequiredIfNotDefault : ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            var property = validationContext.ObjectType.GetProperty("SystemDefault");
            if (property == null)
                return new ValidationResult(string.Format("Unknown property: {0}", "SystemDefault"));
